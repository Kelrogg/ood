...
BOOST_AUTO_TEST_SUITE(WritingReadingCompress)
	...
	BOOST_AUTO_TEST_CASE(Write_10_bytes_read_compressed_test)
	{
		...
		{
			auto oHandler = FilePrinter(TEST_FILE_NAME, std::move(std::vector<unsigned char>{}), true);

			oHandler->WriteBlock(block, BLOCK_SIZE);
		}
		auto iHandler = FileReader(TEST_FILE_NAME, std::move(std::vector<unsigned char>{}), false);
	...
	BOOST_AUTO_TEST_CASE(Write_10_compressed_read_decompressed_test)
	{
		...
		auto iHandler = FileReader(TEST_FILE_NAME, std::move(std::vector<unsigned char>{}), true);
