...
BOOST_AUTO_TEST_SUITE(WritingReadingCrpyt)
	...
	BOOST_AUTO_TEST_CASE(Read_encrypted_and_write_decrypted_10_bytes_test)
	{
		...
		{
			auto oHandler = FilePrinter(TEST_FILE_NAME, std::move(std::vector<unsigned char>{ 0, 1, 2, 255 }), false);

			oHandler->WriteBlock(block, BLOCK_SIZE);
		}
		auto iHandler = FileReader(TEST_FILE_NAME, std::move(std::vector<unsigned char>{ 255, 2, 1, 0 }), false);
